...
interface Step { label: string; stages: TranscriptionStage[] }
...
export default function PipelineStatus({
  stage, progress, llmStep, llmLabel, error
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-card px-4 py-4">
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            <React.Fragment key={step.label}>
              ...
              {i < STEPS.length - 1 && (
                <div className="mx-2 h-px flex-1 bg-border/60" />
              )}
            </React.Fragment>
          )
        })}
      ...
      <p className="mt-2 text-[11px] text-muted-foreground">
        {stage === 'downloading-model' && `Loading Whisper model…${progress > 0 ? ` ${progress}%` : ''}`}
