...
- [../.fallowrc.jsonc](../.fallowrc.jsonc) configures Fallow with explicit Electron-Vite entrypoints:
...
- Run `npx fallow dead-code` to audit unused files/exports/deps without treating Electron-Vite
...
- Ambient declaration files [../src/preload/index.d.ts](../src/preload/index.d.ts) and
  ...
  listed under `ignorePatterns` in `.fallowrc.jsonc` because they are type-only entry surfaces.
